  ...
  outputs = { self, nixpkgs, android-nixpkgs }:
    ...
    in {
      devShells.${system} = {
        ...
        # separate from the default shell: it only needs adb, not the full SDK/Gradle toolchain
        # that AGENTS.md keeps on the remote build hosts.
        screenshots = pkgs.mkShell {
          packages = with pkgs; [ fastlane android-sdk ];
