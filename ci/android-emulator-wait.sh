...
# uses force-avd-creation) that race can show up as the instrumented test's very first click
# failing with "Failed to inject touch input". Wait for a fuller set of readiness signals before
# doing anything else. Ported from the sibling nyetbox project's screenshot/E2E CI.
wait_for_android() {
